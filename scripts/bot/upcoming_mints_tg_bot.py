...
import sys
sys.path.insert(0, str(WORKLOAD_ROOT / "scripts"))
from rich_report import build_table_block, filtered_entry, rich_text_from_markdown
...
class CommandResponse:
    text: str = ""
    rich_blocks: list[dict[str, Any]] | None = None
...
def build_wallet_report_entries(data: dict[str, Any], wallets: list[str], today_only: bool = False) -> list[tuple[str, dict[str, Any], str]]:
    entries = []
    for slug, entry in sorted(data.items(), key=lambda item: extract_earliest_time(item[1]) if isinstance(item[1], dict) else datetime.max.replace(tzinfo=LOCAL_TZ)):
        if not isinstance(entry, dict) or not is_active_entry(entry):
            continue
        filtered = filtered_entry(entry, wallets=wallets, today_only=today_only, stage_today=is_stage_today)
        if filtered is None:
            continue
        header = format_entry(slug, filtered).split("\n", 1)[0]
        entries.append((slug, filtered, header))
...
def build_report_response(title: str, entries: list[tuple[str, dict[str, Any], str]], fallback_text: str) -> CommandResponse:
    wallet_display = load_wallet_display()
    blocks = [{"type": "heading", "text": title, "size": 2}, {"type": "paragraph", "text": "\n"}]
    for index, (_, entry, header) in enumerate(entries):
        if index:
            blocks.append({"type": "paragraph", "text": "\n"})
        blocks.extend([{"type": "paragraph", "text": rich_text_from_markdown(header)}, build_table_block(entry, wallet_display)])
    return CommandResponse(text=fallback_text, rich_blocks=blocks, prefer_rich=bool(entries))


def build_command_response(text: str, data: dict[str, Any], *, user_id: int | str | None = None, chat_id: int | str | None = None, thread_id: int | str | None = None) -> CommandResponse | str | None:
    parsed = parse_command(text)
    plain = handle_command(text, user_id=user_id, chat_id=chat_id, thread_id=thread_id)
    if not parsed or parsed[0] not in {"upcoming", "today", "slug"}:
        return plain
    if isinstance(plain, CommandResponse):
        return plain
    cmd, args = parsed[0], parsed[1]
    wallet_display = load_wallet_display()
    entries = []
    if cmd in {"upcoming", "today"}:
        wallets = resolve_wallet_queries(args, wallet_display)
        if wallets:
            entries = build_wallet_report_entries(data, wallets, today_only=cmd == "today")
    else:
        queries, wallet_filter = parse_slug_args(args, wallet_display)
        for query in queries:
            matches = find_slug_matches(data, query, limit=1)
            if matches:
                slug, entry, _ = matches[0]
                filtered = filtered_entry(entry, wallets=wallet_filter or None, stage_today=is_stage_today)
                if filtered:
                    entries.append((slug, filtered, format_entry(slug, filtered).split("\n", 1)[0]))
    title = "📅 Mint Today" if cmd == "today" else ("🔎 Slug Lookup" if cmd == "slug" else "📋 Upcoming Mints")
    return build_report_response(title, entries, plain or "")
...
def send_rich_message(chat_id: int | str, blocks: list[dict[str, Any]], thread_id: int | None = None) -> bool:
    payload = {"chat_id": chat_id, "rich_message": {"blocks": blocks}}
    if thread_id:
        payload["message_thread_id"] = thread_id
    try:
        result = api_call("sendRichMessage", payload=payload)
        return bool(result)
    except Exception as exc:
        log(f"rich message failed: {exc}")
        return False
...
def handle_check(args: str) -> str | CommandResponse:
    ...
    try:
        ...
        fallback = fuzzy_note + rendered
        return build_report_response("✅ Check Result", [(matched_slug, entry, header)], fallback)
    finally:
        set_service_running(False)
        release_service_file_lock(service_lock_fh)


def handle_command(text: str, *, user_id: int | str | None = None, chat_id: int | str | None = None, thread_id: int | str | None = None) -> str | CommandResponse | None:
    parsed = parse_command(text)
    if not parsed:
        return None
    cmd, args = parsed
        ...
        try:
            ...
            for update in updates:
                ...
                if parse_command(text) and not allowed_thread(thread_id):
                    ...
                    continue
                data = load_json(UPCOMING_FILE, {})
                response = build_command_response(text, data, user_id=user_id, chat_id=chat_id, thread_id=thread_id) if isinstance(data, dict) else handle_command(text, user_id=user_id, chat_id=chat_id, thread_id=thread_id)
                if isinstance(response, CommandResponse):
                    if response.prefer_rich and response.rich_blocks and not send_rich_message(chat_id, response.rich_blocks, thread_id=thread_id):
                        send_message(chat_id, response.text, thread_id=thread_id)
                    elif not response.prefer_rich and response.text:
                        send_message(chat_id, response.text, thread_id=thread_id)
                elif response:
