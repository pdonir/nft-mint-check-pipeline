...
CONFIG_PATH = Path(os.environ.get("NFT_CONFIG_PATH", "/path/to/nft_config.json"))
...
def load_wallet(wallet_name: str, config_path: Path = CONFIG_PATH) -> Dict[str, str]:
    ...
    # Canonical schema stores chain-specific credentials under `evm` and
    # `solana`. Keep legacy flat-schema support for older deployments.
    evm = wallet.get("evm") if isinstance(wallet.get("evm"), dict) else wallet
    if not evm.get("address") or not evm.get("private_key"):
        raise SystemExit(f"Wallet '{wallet_name}' is missing evm.address/evm.private_key")
    return {
        "address": evm["address"],
