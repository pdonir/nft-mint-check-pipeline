...
from rich_report import build_table_block, rich_text_from_markdown
...
def build_today_entries(upcoming):
    """Return Mint Today entries using the same filter as the text report."""
    result = {}
    for slug, entry in upcoming.items():
        wallets = entry.get("wallets", {}) if isinstance(entry.get("wallets"), dict) else {}
        selected = {wallet: stages for wallet, stages in wallets.items() if isinstance(stages, list)}
        if any(is_stage_today(stage) for stages in selected.values() for stage in stages):
            result[slug] = {**entry, "wallets": selected}
    return result


def build_rich_report_blocks(title, projects):
    """Build Rich Message blocks without sending them."""
    blocks = [{"type": "heading", "text": title, "size": 2}, {"type": "paragraph", "text": "\n"}]
    for index, (_, entry, header) in enumerate(projects):
        if index:
            blocks.append({"type": "paragraph", "text": "\n"})
        blocks.append({"type": "paragraph", "text": rich_text_from_markdown(header)})
        blocks.append(build_table_block(entry, WALLET_DISPLAY))
    return blocks
...
def _send_rich_telegram(blocks):
    """Send native Rich Message blocks; caller handles plain-text fallback."""
    if not TELEGRAM_BOT_TOKEN or not TELEGRAM_CHAT_ID or not blocks:
        return False
    payload_data = {"chat_id": TELEGRAM_CHAT_ID, "rich_message": {"blocks": blocks}}
    if TELEGRAM_MESSAGE_THREAD_ID:
        payload_data["message_thread_id"] = int(TELEGRAM_MESSAGE_THREAD_ID)
    url = f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendRichMessage"
    try:
        req = urllib.request.Request(url, data=json.dumps(payload_data, ensure_ascii=False).encode(), headers={"Content-Type": "application/json"})
        with urllib.request.urlopen(req, timeout=15) as resp:
            return bool(json.loads(resp.read().decode()).get("ok"))
    except Exception as exc:
        log(f"[!] Rich Telegram send failed: {exc}")
        return False
...
def main():
    ...
    if upcoming_report:
        projects = [(slug, entry, build_project_header_line(idx, entry, slug=slug)) for idx, (slug, entry) in enumerate(sorted(upcoming.items(), key=lambda x: extract_earliest_time(x[1])), start=1)]
        if not _send_rich_telegram(build_rich_report_blocks("📋 Upcoming Mints", projects)):
    ...
    if today_report:
        ...
        print(today_report)
        log("---")
        today_entries = build_today_entries(upcoming)
        projects = [(slug, entry, build_project_header_line(idx, entry, slug=slug)) for idx, (slug, entry) in enumerate(sorted(today_entries.items(), key=lambda x: extract_earliest_time(x[1])), start=1)]
        if not _send_rich_telegram(build_rich_report_blocks(f"📅 Mint Today ({datetime.now(LOCAL_TZ).strftime('%d %b')})", projects)):
