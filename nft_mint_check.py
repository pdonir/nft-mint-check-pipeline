...
OPENSEA_CHECKER_DIR = BASE_DIR / "opensea_checker"
ELIG_SCRIPT = str(OPENSEA_CHECKER_DIR / "check_eligibility_api_batch.py")
ELIG_FALLBACK_SCRIPT = str(BASE_DIR.parent / "opensea_automation" / "check_eligibility_batch.py")
CUSTOM_ELIG_SCRIPT = str(BASE_DIR / "check_custom_eligibility.py")
SCRAPER_SCRIPT = str(BASE_DIR / "x_list_scraper.py")
COOKIES_FILE = str(BASE_DIR.parent / "twitter_cookies.json")
OPENSEA_API_ERROR_LOG = BASE_DIR / "opensea_api_errors.log"
...
def run_eligibility_check(slugs):
    ...
    if result is None or result.returncode != 0:
        ...
        try:
            result = subprocess.run(
                ...
                cwd=str(BASE_DIR.parent / "opensea_automation")
            )
        except subprocess.TimeoutExpired:
    ...
    with open(BASE_DIR / "last_elig_report.txt", 'w') as f:
        f.write(report_text)
...
def main():
    ...
    # ── Load scraped links ──
    scraped_file = BASE_DIR / "scraped_links.json"
    slugs = []
    custom_project_keys = []
